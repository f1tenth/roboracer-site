...
3. No binary over 1.5 MB enters git, with one standing exception approved by Cedric (2026-08-20, revised 2026-08-21): the landing hero loop encodes committed under `public/media/hero/` (the desktop 1280 encode up to 8 MB; the mobile 960 encode under 3 MB; poster required; native resolution only, no upscaling). There is no Cloudflare account; all other video and large GIFs are cut to fit, reduced to posters, or dropped (see skill `roboracer-media`). Images are compressed to WebP/AVIF with a poster for every video. Existing oversized files in `public/landing` get replaced, not duplicated.
...
8. Do not copy assets (images, video, SVG, copy) from neobotics.org or any third-party site. Neobotics is a pattern reference only. One standing exception (Cedric, 2026-09-25): the spinoff features on `/about` (Neobotics, Quanser) may use those companies' own logos, car images and site imagery, framed as a summarized preview of their website; every file gets a manifest row with its source URL and permission status. Media from community LinkedIn posts is used only after permission is recorded in the media manifest.
