...
type ContributorStripProps = {
  ...
  emptyLabel: string;
  /** False inside a closed <details>: a scroll-triggered entrance measured
   * while folded could leave the strip hidden when it opens. */
  reveal?: boolean;
};
...
export default function ContributorStrip({
  ...
  const chips = contributors.map((c) => (
