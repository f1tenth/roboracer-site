...
export type SocialNetwork = "slack" | "linkedin" | "instagram" | "github";

/**
 * Brand glyphs from Simple Icons (CC0 1.0), inlined so the site carries no
 * icon dependency: Instagram and GitHub from the current set, Slack and
 * LinkedIn from simple-icons 9.21.0 (both were removed from later releases
 * at the brands' request; the 9.x files are CC0). All on a 24x24 box.
 */
  ...
  if (soon) {
    return (
      <a
        ...
        aria-disabled="true"
        onClick={stop}
        className={`${buttonClasses(variant, "paper", "md")} cursor-default ${className}`}
      >
        {glyph}
        <span>{children}</span>
