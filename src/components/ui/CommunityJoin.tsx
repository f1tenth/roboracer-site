...
import { loadCommunity, type Community, type CommunityPost, type JoinPost, type JoinYouTube } from "../../lib/data";
    ...
    <Section edge rule width="page" id="join" aria-labelledby="join-title" className={className}>
      ...
      {(post || youtube) && (
        <div className="mt-14 border-t border-ink-950/10 pt-8">
          ...
          {youtube && <YouTubeCard yt={youtube} full />}
        </div>
      )}
    </Section>
  );
...
function YouTubeCard({ yt, full = false }: { yt: JoinYouTube; full?: boolean }) {
  ...
  return (
    <article
      ...
      className={
        full
          ? "relative left-1/2 mt-10 w-screen -translate-x-1/2 border-y border-ink-950/10"
