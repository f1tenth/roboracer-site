...
// `number` is the official "Nth" of the competition series and feeds the
// chapter's ticker; two race entries sharing one is a data error to fix by hand.
const numbered = new Map();
for (const e of source.events) {
  if (e.kind !== "race" || typeof e.number !== "number") continue;
  if (numbered.has(e.number)) console.warn(`duplicate number ${e.number}: ${numbered.get(e.number)} and ${e.id}`);
  numbered.set(e.number, e.id);
}
