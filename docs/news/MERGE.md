...
Since then (for the current count, not the merge): the media branch added `icra2026-sim-league-monacof1-wins` (76 items), and on 2026-09-26 Cedric removed `iros2026-registration-closes-sep-5` and `ifac2026-busan-opens`. news.json now holds **74 items** (2026: 17); 73 carry a picture or a video, and the one without is `cpsweek2024-hongkong-fsm-speed-wins`.
...
Added 2026-09-26 on `revamp/p3-news-media` (taskmap n338), from `docs/news/MEDIA_PLAN.md`, for Cedric's note that every item should aim for a picture and that the Sim Racing League items should carry the Tinker Twins' recap videos (he approved YouTube embeds). Videos only on this branch; the photo and poster items are a second agent's job (`docs/news/MEDIA_IMAGES.json`).

- **Code.** `embed` now takes `{ provider: "youtube", id, title, poster? }` beside the LinkedIn one. A card shows the poster (the item's `image`, else `poster`, else the video's i.ytimg.com `maxresdefault` thumbnail) with a play disc; "Play the video" under the text, or a click on the poster, mounts the youtube-nocookie.com player in place (titled iframe, nothing but the thumbnail loads before a click, nothing self-starts). The lead renders a YouTube embed in its picture layout. An item whose `embed` is already its LinkedIn post takes the video in a new `video` field instead (the card shows both, the lead puts it under its links). The player is `YouTubePlayer`, extracted from the About page's `YouTubeFacade` so both use one player.
- **Counts.** 34 items carry a YouTube video: the plan's 20 `add` items, 7 `upgrade` items (today's picture is the poster), 6 `secondary` items (3 as `embed` with the picture as poster, 3 as `video` beside their LinkedIn post: `ifac2026-busan-largest-race`, `icra2023-london-forzaeth-wins`, `f110-simulator-open-source-2019`), and the new `icra2026-sim-league-monacof1-wins`. Items with media: 59 of 76 (was 38 of 75); after the photo pass 73 of 76, and after the two removals of 2026-09-26, 73 of 74. The 17 without are the plan's 11 `photo` and 4 `poster` items (second agent) and its 2 `none` items. Every `credit` names the channel as `Video: <channel> · YouTube`; `sources` are unchanged except on the new item.
- **New item.** `icra2026-sim-league-monacof1-wins` (fifth Sim Racing League, ICRA 2026, `status: "verify"`): not in the candidates or the content skill, so every fact comes from AutoDRIVE's own ICRA 2026 league page (73 registered, 28 qualified, MonacoF1 of Ekumen first in 110.27 s ahead of RUN-RUN-ChuraTaro and bracavisionai) and the recap video xCp1KB-JGZ4 (73 teams, 200+ participants). Month precision (May 2026): the page's timeline is marked "subject to change".
- **Cedric must approve** (from MEDIA_PLAN.md, not acted on here): item 7, the third-party channels now embedded (AiX, TU Wien TV, D-ITET PBL, Madhur Behl, Pusan National University, The Robotics Club, VAUL, Columbia Video Network, Unicorn Racing, industrialinformatics); item 10, the five older races now led by their recap with today's photo as poster; the Busan lead's `video` is the host's seven-hour race-day stream (drop the field to restore the lead exactly); the new fifth-league item; the sixth league's LinkedIn embed stays until AutoDRIVE posts its recap. Items 1 to 6 and 9 (photos, posters, the two `none` items) belong to the photo pass.
...
8. 2026 opens with four text cards and its photo posts sit behind the year's fold (items stay newest first within a year). Default: keep newest first. Settled by Cedric 2026-09-26: 2026 opens with 14 items, through UPenn's ICRA 2026 post; only the ICRA 2026 results and the fifth Sim Racing League fold.
