...
| | |
|---|---|
| Working branch | none open. The last was `revamp/news-2026` (**PR #22**, merged 2026-09-26, `e6aae43`, live on roboracer.ai): Cedric removed the IROS 2026 registration notice and the IFAC 2026 "opens" notice from `news.json` (74 items now), and the newest year on `/news` opens with 14 so its visible run ends at UPenn's ICRA 2026 post (`docs/qa/news-2026.md`), and the Siga Siga and VTC 2026 items carry the pictures from AutoDRIVE's and Neobotics' own posts (manifest NEWS-25 and NEWS-26, permission not yet requested; a card shows a banner wider than 2.5:1 whole; `docs/qa/news-photos.md`). Before it: iteration 3 shipped with **PR #20** (merged 2026-09-26, `1800df2`, live on roboracer.ai the same hour). Start the next iteration from `main` with `scripts/rr.sh page <name>`. What PR #20 carried (iteration 3, 2026-09-25/26, from Cedric's review notes): verify tags gone, team facts from the `RoboRacer_Teams_DB` sheet (`docs/content/teams.sheet.json`), Neobotics and Quanser as website-preview features under "Related platforms", every route change opens at the top (`hooks/useRouteScroll.ts`, hash landings clear the nav), `/research` with full-width phone figures and per-year folds, `/race` leaderboard on follow the gap with the top-5 replay, one "Start here" section replacing Start here + Platform on `/` (`ui/StartHere`; `/about` opens on "What RoboRacer is" and links to it), the research carousel figure filling its pane, the Join section leading with copy, and the news history 2016 to 2026 (76 items at the merge; 74 since Cedric removed two 2026 notices on 2026-09-26, 73 of them with a picture or video: 34 YouTube recaps behind a click-to-play poster, `docs/news/MEDIA_PLAN.md`). Follow-ups the same day: the replay on `/race` plays by itself once in view; the landing's Start here is a two-column grid of landscape clip tiles with the header in the first cell, sized to fit one laptop screen (`docs/qa/p3-start-fit.md`); `/about` opens with "What RoboRacer is" and no longer repeats Start here; "Related platforms" replaces "Spinoffs" (Quanser stated as unaffiliated). The topic branches `revamp/p3-*` and `revamp/polish-2` are merged and kept, with their worktrees in `../roboracer-site-wt/p3-*` (QA PNGs live there, git-ignored). |
| `main` | at `e6aae43` (PR #22 on top of PR #20, merged 2026-09-26) = the live roboracer.ai. **Never commit or push to it.** Cedric merges; the merge deploys. |
...
|---|---|---|---|
| `/` | `src/pages/Landing.tsx` | **v1.1** | The reference for the visual system. Hero clip cycle loads one clip at a time and drops to 960 on slow links (`HeroChapter.tsx`, `lib/media.ts`); "00 Start here" after the hero: one line on what RoboRacer is, then Build, Learn, Race, Research, Sponsor as a grid of tiles with their clips from `desktop:` (thumb rows on phones), from `paths.json` (`ui/StartHere.tsx`; it replaced the entry-path row and the pinned Platform chapter, 2026-09-25, and is the one place the five appear: the nav's "Start here" goes to `/#start` from every route); sections 00 to 08; chapters unpinned on `compact:` |
