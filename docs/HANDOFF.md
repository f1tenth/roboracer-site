...
| `/about` | `About.tsx` | **revamped** | 01 Start here: the story beside the ICRA group photo, then the landing's ways in, fuller (`ui/StartHere` `density="full"`: clip, caption and a second sentence; phones get the thumbs); 04 Spinoffs from `spinoffs.json` (`about/SpinoffGrid.tsx`; entries without an origin are not rendered), 05 Videos (`videos.json`, click-to-load `YouTubeFacade`), 06 the shared Join block |
